...
class DatabaseManager:
    ...
    def get_random_prize(self):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute("SELECT prize_id FROM prizes WHERE used = 0 ORDER BY RANDOM() LIMIT")
        return cur.fetchall()[0]
    

    def get_winners_count(self, prize_id):# возвращает количество пользователей, которые выиграли приз
        conn = sqlite3.connect(self.database)
        with conn:
            cur = conn.cursor()
    ...
    #возвращает таблицу (имя пользователя и количество призов, которые он получил). Данные в таблице отсортированы в порядке убывания. Максимум в списке должно оказаться 10 победеителей.   
    def get_rating(self):
        conn = sqlite3.connect(self.database)
        with conn:
            cur = conn.cursor()
        ...
        count(winners.prize_id) as count_prize
        FROM winners
